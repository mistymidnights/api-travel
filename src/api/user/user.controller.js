...
const register = async (req, res, next)=>{
    try {
        ...
        const nicknameExist = await User.findOne({nickname: newUser.nickname});
        if (emailExist || nicknameExist) return next(setError(409, 'This email || userName already exist'));
        if (req.file) newUser.img = req.file.path;
